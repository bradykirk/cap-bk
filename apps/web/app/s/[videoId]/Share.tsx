...
const UNSTARTED_TRANSCRIPTION_TIMEOUT_MS = 5 * 60 * 1000;
...
export const Share = ({
	...
	const [unstartedTranscriptionTimedOut, setUnstartedTranscriptionTimedOut] =
		useState(false);

	useEffect(() => {
		setUnstartedTranscriptionTimedOut(false);
		if (transcriptionStatus) return;

		const timer = setTimeout(
			() => setUnstartedTranscriptionTimedOut(true),
			UNSTARTED_TRANSCRIPTION_TIMEOUT_MS,
		);
